...
namespace BillingRegister.Controllers
{
    ...
    public class GroupsController : Controller
    {
        ...
        {
            var GroupsList = this.GroupManager.Groups.ToList();
            var ss = GroupsList.OrderBy(n => n.Name);
            return View(ss);
        }
        ...
        public async Task<ActionResult> groupRole(string id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
        ...
        public async Task<ActionResult> groupRole(
             [Bind(Include = "Id")] GroupViewModel model, params string[] selectedRoles)
        //[Bind(Include = "Id,Name,Description")] GroupViewModel model, params string[] selectedRoles)
        {
            var group = await this.GroupManager.FindByIdAsync(model.Id);
            if (group == null)
